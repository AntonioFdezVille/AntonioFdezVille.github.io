document.addEventListener('DOMContentLoaded', () => {
    ...
    // Llama a la función auxiliar para obtener el logo de forma local.
    // Asegúrate de tener la función getLogoBase64() y la etiqueta <img> en tu código.
    ...
    // Añade la imagen si se cargó correctamente.
    if (logoBase64) {
        doc.addImage(logoBase64, 'JPEG', 40, 40, 120, 120);
    }
    ...
    // --- CORRECCIÓN AQUÍ ---
    // 1. Cargamos el logo dinámicamente desde el HTML.
    const logoBase64 = await getLogoBase64(); 
    ...
    const dibujarCabecera = (doc, yStart = 40) => {
        // --- CORRECCIÓN AQUÍ ---
        // 2. Usamos el logo y ajustamos su tamaño. 
        //    Puedes cambiar 120, 120 por el tamaño que prefieras.
        if (logoBase64) {
             doc.addImage(logoBase64, 'jpeg', 40, yStart, 120, 120);
        }
...
/**
 * Carga la imagen del logo desde el HTML y la convierte a formato Base64.
 * Es un proceso 100% local que no usa internet.
 * Devuelve una "promesa" con los datos de la imagen listos para el PDF.
 */
function getLogoBase64() {
    return new Promise((resolve, reject) => {
        const img = document.getElementById('logo-pdf');
        
        // Si la imagen ya se cargó, la usamos
        if (img.complete) {
            ...
            // Si no, esperamos a que termine de cargar
            img.onload = () => {
                const canvas = document.createElement('canvas');
                canvas.width = img.naturalWidth;
                canvas.height = img.naturalHeight;
            ...
            img.onerror = () => {
                reject(new Error('Error local: No se pudo cargar la imagen desde la ruta.'));
            };
        }
    });
}